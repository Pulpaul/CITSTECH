                        ...
                        <?php
                              if ($row['user_image'] == "") {
                                echo "<img src='img/avatar.jpg' alt='Circle Image' class='rounded-circle img-fluid'>";
                                  ...
                                  echo "<img src='data: user_image/jpeg;base64,".base64_encode($row['user_image'])."' alt='Circle Image' class='rounded-circle img-fluid'>";  
                                } 
                              ?>
                        </div>
                    ...
                    <div class="card-body"> 
                      ...
                      <table id="myTable" class="table table-hover table-striped">
                        ...
                        <?php
                            ...
                            $query = mysqli_query($conn,"SELECT * FROM tblreply WHERE users_id='$user_id' AND status = 'Trashed' ");
                            $row = mysqli_fetch_array($query);
                            $rowwqww = mysqli_fetch_array($query);

                            if ($row == 0) {
                              $blank = "<div class='alert alert-default text-center' style='background: #EEE;'><label>Trash is Empty</label></div>";
                            }
                            else {
                            $query = "SELECT * FROM tblreply INNER JOIN register_user ON sup_id = register_user_id WHERE users_id='$user_id' AND status = 'Trashed' ORDER BY time_reply DESC ";
                          ...
                          <tr>  
                            ...
                            <td><?php echo $row['rep_concern']; ?></td>
                             <td>
                              <?php 
                              if ($row['rep_image'] == "") {
                                echo "N/A";
                                ...
                                echo "<img src='data: image/jpeg;base64,".base64_encode($row['rep_image'])."' class='img-raised rounded img-fluid' width='75'>"; 
                              }
                              ?>
                              </td>
                             <td><?php echo $row['date_reply']; ?></td>
                          <td class="text-center">
                            <button class="btn btn-primary btn-sm btn-fab btn-round" data-toggle="tooltip" title="Restore" id="restoreReply" data-id="<?php echo $row['rep_id']; ?>"><i class="fa fa-undo"></i></a>
                            <button class="btn btn-warning btn-sm btn-fab btn-round" data-toggle="tooltip" title="Delete" id="deleteReply" data-id="<?php echo $row['rep_id']; ?>"><i class="fa fa-archive"></i></a>

                            <a class="btn btn-default btn-sm btn-fab btn-round" href="php/deleteConcern.php?id=<?php echo $row['id']; ?>" data-toggle="tooltip" title="Delete" hidden><i class="fa fa-archive"></i></a>
                          </td>
                        <?php } } ?>
                        <?php 
                            ...
                            $query = mysqli_query($conn,"SELECT * FROM tblconcern WHERE user_id = '$user_id' AND status = 'Trashed' ");
                            $row = mysqli_fetch_array($query);

                            $qwe = mysqli_query($conn,"SELECT * FROM tblreply WHERE users_id='$user_id' AND status = 'Trashed' "); 
                            $rowwqww = mysqli_fetch_array($qwe);

                            if ($row == 0 && $rowwqww == 0) {
                              $blank = "<div class='alert alert-default text-center' style='background: #EEE;'><label>Trash is Empty</label></div>";
                            }
                            else {
                            $query = "SELECT * FROM tblconcern INNER JOIN register_user ON user_id = register_user_id WHERE user_id='$user_id' AND status = 'Trashed' ORDER BY time_sent DESC ";
                            $result = mysqli_query($conn, $query);
                            while ($row = mysqli_fetch_array($result)) { 
                          ...
                          <td class="text-center">
                            <button class="btn btn-primary btn-sm btn-fab btn-round" data-toggle="tooltip" title="Restore" id="restoreOutbox" data-id="<?php echo $row['id']; ?>"><i class="fa fa-undo"></i></a>
                            <button class="btn btn-warning btn-sm btn-fab btn-round" data-toggle="tooltip" title="Delete" id="deleteOutbox" data-id="<?php echo $row['id']; ?>"><i class="fa fa-archive"></i></a>
